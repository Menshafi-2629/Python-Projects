...
def add_to_list(d,z,q,p):
    if d[2] == 0:
        ...
        print("="*63)
        print(f"{d[0]:>5} | {d[1]:<13} | {d[3]:>11} | {z:>8} | {expiry(q)}")
    else:
        p.append([d,z])
        print(f"{'ID':>5} | {'Name':<13} | {'Price':>11} | {'Quantity':>8} | {'E.C':<13}")
        print("="*63)
...
def bill_generation(p,t,status):
    with open("Super_Market_Bill.txt","w")as f:
        heading = f"{'ID':>5} | {'Name':<13} | {'Price':>12} | {'Quantity':>8} | {'E.C':<13}"
        f.write(heading + "\n")
        border="="*64
        f.write(border + "\n")
    with open("Super_Market_Bill.txt","a")as f:
        for i,z in p:
            line=f"{i[0]:>5} | {i[1]:<13} | {i[3]:>12} | {z:>8} | {expiry(i[0])}\n"
            f.write(line)
        price=f"\nTotal Price: {t}"
        f.write(price+"\n")
        f.write(status)
    os.startfile("Super_Market_Bill.txt","print")
    ...
    while True:
        ...
        if a=='y':
            ...
            if p:  
                ...
                print("="*64)
                for i,z in p:
                    print(f"{i[0]:>5} | {i[1]:<13} | {i[3]:>12} | {z:>8} | {expiry(i[0])}")
                print("\nTotal Price:",t)
                while True:
                    while True:
                        try:
                ...
                if a=='y':
                    ...
                    db.commit()
                    print("Purchase completed. Stocks updated")
                    while True:
                        while True:
                            try:
                                a=input("\n\nDo you want to generate the bill (y/n): ").lower()
                                break
                            except:
                                print("Please enter 'y' or 'n'")
                        if a!='y' and a!='n':
                    ...
                    while True:
                        ...
                        if a!='y' and a!='n':
                            ...
                            break
                    if a=='y':
                        bill_generation(p,t,"Bill not paid")
                    else:
                        print("Bill not generated\n\n\n")
                t=0
                p.clear()
